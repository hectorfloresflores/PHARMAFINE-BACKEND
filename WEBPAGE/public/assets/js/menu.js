...
document.addEventListener("click", function(event) {
  ...
  switch (event.target.id) {
    case "ph_sign_in":
      ...
      if (emailLabel.value == "" || emailPassword.value == "") {
      ...
      xhr.onload = function() {
        switch (xhr.status) {
          ...
          case 200:
            ...
            xhr.open(
              "GET",
              `http://localhost:3002/sendEmailConfirmation?token=${response.token}&email=${registerEmail.value}`
            );
            xhr.setRequestHeader("Content-Type", "application/json");
            xhr.send();
            xhr.onload = function() {
